...
export async function GET(request: Request) {
  ...
  try {
    ...
    const revealSensitive = canManageHr && (await isPrivacyUnlockedForOrg({ request, orgId: org.id, module: "hr" }))
    return NextResponse.json({
      payroll: payroll.map((record) =>
        revealSensitive
          ...
          : {
              ...record,
              employeeName: "Protected employee",
              baseSalary: 0,
              bonus: 0,
              deductions: 0,
              netPay: 0,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return handleAccessRouteError(error, "Failed to create payroll record")
  }
}
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    return handleAccessRouteError(error, "Failed to update payroll record")
  }
}
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return handleAccessRouteError(error, "Failed to delete payroll record")
  }
}
