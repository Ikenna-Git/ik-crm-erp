...
| HR privacy lock posture | HR UI + API | Fixed in code, evidence pending | Root cause was client-side unlock gating that ignored server `canUnlock`. HR now uses server-authorized unlock state and keeps redaction intact. | Locked, wrong PIN, unlocked, View Details, re-lock live evidence |
| Accounting privacy lock posture | Accounting UI + API | Fixed in code, evidence pending | Root cause was client-side unlock gating that ignored server `canUnlock`. Accounting now uses server-authorized unlock state and keeps redaction/export blocking intact. | Locked, wrong PIN, unlocked, export, approval, re-lock live evidence |
